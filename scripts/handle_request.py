...
class HandleRequest:
    ...
    def send(self, url, method="post", data=None, is_json=True, **kwargs):
        ...
        method = method.lower()
        if method in "get":  # 如果为get请求, 那么传递的data, 默认传查询字符串参数
...
if __name__ == '__main__':
    # 1. 构造请求的url
    login_url = "http://182.61.1.2:9101/api/auth/login"
    recharge_url = "http://182.61.1.2:9101/api/product/busproduct/goods/1283671524322054144"
    ...
    login_params = {"phone":"18911111113","password":"aaa123"}
    ...
    if "token" in login_res.text:
        token = json_datas.get('data')['token']  # 获取token信息
    ...
    # 5. 获取产品列表
    recharge_res = do_request.send(recharge_url,method='delete', data=recharge_params)
